...
import type { CuratedMemoryDto, SessionLogResult } from '../api/types';
...
export default function AgentMemory() {
  ...
  const [searchResults, setSearchResults] = useState<SessionLogResult[] | null>(null);
  const [searchError, setSearchError] = useState<string | null>(null);

  useEffect(() => {
    setCurated(null);
    setCuratedError(null);
    getCuratedMemories(agentId)
      .then(setCurated)
  ...
  const handleSearch = async () => {
    ...
    setSearchError(null);
    try {
      const results = await searchMemory(agentId, searchQuery);
    ...
    <>
      ...
      <div className="flex justify-center mb-6">
        <div className="segmented-group">
          <button
            className={activeTab === 'curated' ? 'segmented-btn-active' : 'segmented-btn'}
            onClick={() => setActiveTab('curated')}
          >
            <span className="material-symbols-outlined text-lg mr-1.5">bookmark</span>
            {t('memory.curatedMemory')}
          </button>
          <button
            className={activeTab === 'search' ? 'segmented-btn-active' : 'segmented-btn'}
            onClick={() => setActiveTab('search')}
          >
            <span className="material-symbols-outlined text-lg mr-1.5">search</span>
            {t('memory.searchLogs')}
          </button>
        </div>
      </div>
      ...
      {activeTab === 'curated' ? (
        curatedError ? (
          ...
          <div className="empty-state">
            <span className="material-symbols-outlined empty-state-icon">memory</span>
            <p className="empty-state-text">{t('memory.noCurated')}</p>
          </div>
        ) : (
            ...
            searchResults.length === 0 ? (
              ...
              <div className="space-y-3">
                <p className="text-label-lg text-on-surface-variant mb-2">
                  <span className="material-symbols-outlined text-lg mr-1 align-middle">info</span>
                  ...
                  <div key={log.id} className="card-outlined">
                    <div className="flex justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-label-lg text-on-surface">{log.session_id}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="badge-neutral text-label-sm">{log.log_type}</span>
                        <span className="text-body-sm text-on-surface-variant">{log.created_at}</span>
                      </div>
                    </div>
