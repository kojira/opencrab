...
14. **スキル呼び出しの自然言語対応（プロンプト改善）**
    - 現状: サブエージェントがゲートウェイアクション名・パラメータを細かく指定しないと動かない
    - 目標: 「天気を調べるスキルを作って」「東京の天気は？」だけでかいろが自分で適切なアクションを選べる
    - 方針: `create_skill`・`execute_skill`のdescriptionとプロンプト例を改善

15. **自律的複合タスク実行の設計（実装なし）**
