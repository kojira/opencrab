...
### 5.2 Claude Codeを使ったパス修正（テンポラリエージェント方式）

スクリプトのパス修正はClaude Codeに丸投げする。パス検出・書き換えだけでなく、外部依存ファイルの対処も文脈込みで判断できる。

**実装方針**:
1. スクリプトファイルをエージェントのワークスペースにコピー（Rust側で機械的に実行）
2. 最小設定のテンポラリエージェントを一時作成
3. そのエージェントが`spawn_coding_agent(claude)`を呼び出し:
   - 指示: 「このスクリプトをワークスペース（`{new_agent_workspace}`）の範囲内で動くように直して」
   - Claude Codeがファイルを調べてパスを書き換え・依存ファイルを確認・必要に応じてコピー
4. 作業完了後にテンポラリエージェントを削除
5. インポート完了レポートに「修正されたスクリプト一覧」を表示

**フォールバック**:
- Claude Codeが失敗した場合は単純な文字列置換（`str.replace(source_dir, new_workspace)`）にフォールバック
- 変換後も元パスが残っている場合はwarningとして報告し、手動修正を推奨

### 5.3 スクリプトファイルのコピー

スキルディレクトリ内にSKILL.md以外のスクリプトファイルが存在する場合：
1. 新しいエージェントのワークスペース（`data/agents/{agent_id}/workspace/`）にコピー（Rust側）
2. テンポラリエージェント経由でClaude Codeがパスを修正
