...
### 5.2 テンポラリエージェント経由のClaude Codeによるパス変換

LLMによる直接テキスト置換ではなく、テンポラリエージェントがClaude Codeを実行してパスを書き換える方式を採用する。これにより、複雑なパス生成ロジックを含むスクリプトでも確実に変換できる。

**フロー**:

1. **スクリプトファイルのコピー（Rust側）**
   - インポートAPIがスキルディレクトリ内のスクリプトファイル（`.py`, `.sh`, `.js`, `.ts`）を新しいエージェントのワークスペース（`data/agents/{agent_id}/workspace/`）にコピーする
   - この時点ではパスはまだ元のまま
...
4. **テンポラリエージェントの削除**
   - パス変換完了後にテンポラリエージェントを削除する
   - 変換結果（書き換えたファイル数・警告等）をインポートAPIに返す

**テンポラリエージェントの仕様**:
- 名前: `__import_helper_{import_id}` （インポートID付きで一意に識別）
- 設定: 最小限（soul/identityは空、スキルなし）
- ライフサイクル: パス変換タスクの完了または失敗時に即削除
- 権限: コピー済みワークスペースディレクトリのみアクセス可

### 5.3 スクリプトファイルのコピー

スキルディレクトリ内にSKILL.md以外のスクリプトファイルが存在する場合：
1. 新しいエージェントのワークスペース（`data/agents/{agent_id}/workspace/`）にコピー
2. テンポラリエージェント経由でClaude Codeによるパス変換を適用
3. インポート完了レポートに「コピーされたスクリプト一覧」と「パス変換の結果」を表示

### 5.4 エラーハンドリング

- テンポラリエージェント作成失敗時: スクリプトコピーはそのまま継続し、warningとして報告
- Claude Code実行失敗時: フォールバックとして単純な文字列置換（`str.replace(source_dir, new_workspace)`）を試行
- 変換後も元パスが残っている場合: warningとしてインポート完了レポートに記載
- テンポラリエージェント削除失敗時: ログに記録し、孤立エージェントのクリーンアップタスクに委ねる
