...
### 5.3 非同期バックグラウンド処理の仕様通知

エージェントがツールコールを含む応答を返した瞬間、システムが自動的にバックグラウンド処理を起動する。
エージェントはこの仕組みを把握しておく必要があるため、システムプロンプトに以下を含めること：

```
ツールを使い始めると、システムが自動的にバックグラウンドタスクを起動する。
エージェントはテキストを返した後に一旦終了し、バックグラウンドでツール処理が続く。
処理完了後、エージェントは再度呼び出されてサブタスクの結果を受け取る。
spawn_subtaskを含むツールコールの結果は即座に返るが（status: spawned等）、
実際の処理結果は非同期にセッション履歴に追加され後から届く。
```

**理由:**
- LLMが「ツールコール後も自分がまだ動いている」と誤解しないため
- `spawn_subtask`を呼んだ後の正しい動作（ackテキストを返して終了）をLLMに理解させるため
