...
- **直近コンテキスト**（必要最小限）
- **自己認知情報**（必須）: サブ起動時にシステムメッセージとして以下を渡す
  ```
...
| 制約 | 内容 | 対応 |
|------|------|------|
| キャンセルはLLM依存 | 「キャンセルして」でcancel_subtaskが呼ばれるかはLLMの判断次第（100%保証不可） | Phase 1: LLMに任せる。Phase 2以降でシステム的な保証を検討 |
| 複数サブ並列の競合 | 同一チャンネルで複数サブが同時に完了した場合の応答順序は未保証 | Phase 2のチャンネル競合管理で対応 |
| デーモン型長期監視 | タイムアウト30分を超える常駐プロセス（nostaro watch等） | Phase 2のexecute_shell(background=true) + heartbeat連携で対応 |
