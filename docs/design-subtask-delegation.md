...
### 5.4 spawn_subtaskは非同期アクション

`spawn_subtask`は他のツール（`execute_shell`等）と異なり**非同期アクション**であることを、システムプロンプトに明記すること：

```
spawn_subtaskは非同期アクションである：
- 呼び出すとすぐにtool_resultが返る（status: spawned）が、これは「起動確認」にすぎない
- 実際の処理はバックグラウンドで継続し、結果は後のターンでセッション履歴に届く
- 処理が完了すると、メインエンジンが自動的に再呼び出しされて結果を受け取る
```

**この仕様を明記する理由:**
- `execute_shell`等は同期的に完了を待つが、`spawn_subtask`は即リターンする非同期アクション
- エージェントが「tool_result = 処理完了」と誤解して次の処理を進めてしまうのを防ぐ
