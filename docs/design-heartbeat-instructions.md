...
- **システムプロンプト注入の是非**: ハートビート指示をシステムプロンプト側にも出すと、通常会話tickに漏れるリスク。v1はセッションログ注入のみで回避するが、長期記憶への影響は要観察。
  - **指示文の重複蓄積（#501, 解決済み）**: 上記のセッションログ注入は毎tick同一文面の `system` 行（`speaker_id='heartbeat'`）を積むため、放置すると以後ずっと会話へ再注入される（本番の heartbeat セッションで会話組み立てが 417K→311K トークン、約 25% がこの重複だった）。会話再構成（`process.rs::retain_conversation_logs`）で**最新の 1 件だけ残す**ようにして解消。subtask 完了本文（`system` かつ `speaker_id=None`, #404/#405）は畳まない。書き込み側（`scheduler.rs`）は変えていない（既存履歴を壊さないため、読み出し側で畳む）。
