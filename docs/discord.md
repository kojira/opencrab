...
`${OWNER_DISCORD_ID}` が未定義で空文字に展開された）場合、「オーナーが居ない」
のではなく**制限が緩む**方向に倒れる。設定ファイルを見ても気づけないので注意する。

| 影響 | 内容 |
|---|---|
| オーナー専用機能 | 誰も owner と判定されないため全て使えなくなる |
| DM の受け付け | 信頼ユーザーが 1 件も登録されていないエージェントでは、**誰からの DM でも受け付ける**（下記「1. DM 制御」参照） |
| オーナー専用 UI | フォーム/モーダル/ボタンの操作者チェックがスキップされ、見えている人なら誰でも操作できる |

**本番では必ずオーナーを設定すること。** 空のまま起動した場合、サーバーは起動時に
`tracing::warn!` で警告を出す。警告が出る条件は経路ごとに次の通り。
...
ゲートウェイは起動しないため、共有側の警告は出ない。

なお、未設定時に制限が緩む方向へ倒れる挙動を fail-closed に統一する作業は #174 で扱う。
...
| 空文字 `""`（未設定） | 0 件 | **すべてのユーザーからの DM を受け付ける**（制限なし） |
| 空文字 `""`（未設定） | 1 件以上 | 信頼ユーザーからの DM のみ応答 |

空文字は「オーナー無しなので誰も特別扱いしない」ではなく、信頼ユーザーが未登録の
エージェントでは**制限が外れる**方向に働く。本番では必ず設定すること（#174）。
