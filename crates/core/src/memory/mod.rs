...
pub mod daily_log_indexer;
pub use daily_log_indexer::{DailyLogIndexStats, DailyLogIndexer};
...
mod tests {
    ...
    fn test_append_and_search_session_log() {
        let mm = test_mm();
        mm.append_session_log("s1", "message", "Rust is great", None, Some(1), None)
            .unwrap();
        mm.append_session_log("s1", "message", "Python is fine", None, Some(2), None)
            .unwrap();
        mm.append_session_log("s1", "message", "Java is verbose", None, Some(3), None)
            .unwrap();
