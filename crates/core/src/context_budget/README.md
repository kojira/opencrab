...
- ターン途中: SkillEngine の各 append で `TokenLedger` 合計だけを更新し、高水位超過のときだけ合成 user 文字列を低水位まで刈る。実行前に各 tool の result 枠を予約する。`ws_read` は `line_count` を 2000 行=30k の既存比で按分し、args から見積もれない読みは 30k、その他は 2.5k。収まらなければ先に刈り、それでも駄目なら副作用 tool を開始せず `context_budget_exhausted`。
