...
// 旧 crates/server から移植したバリアント名をそのまま保つ（#767・faithful port）。
...
// 書き系（Phase 2 の schedule CRUD）で使う。読み取り移植（#767 Phase 1）では未使用だが、
// 旧 crates/server の module を faithful に保つため残す。
#[allow(dead_code)]
