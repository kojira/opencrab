...
- `POST /api/agents/{id}/messages` は `AgentDirectMessage`（スライス 6 の place 機構 + said）のあと Spoke を待って本体封筒 `{session_id,caller_type,responses:[{agent_id,content}]}` を返す。subject→legacy UUID は `resolve_legacy_agent_id`。不在 agent は 404 `{"error":"agent not found: {id}"}`。
...
- `POST /api/sessions` は `PlaceCreateLegacy`（`{theme,mode?,participant_ids,max_turns?}` → `{id}`。未解決 participant は 400。重複 participant_ids は畳んで 200）。`POST /api/sessions/{id}/mentor` は `PrivateJournalAppendMentor`（`{content}` → `{id}`。events に載せない）。`POST /api/sessions/{id}/messages` は 501。値つき未復元キーは 501。
