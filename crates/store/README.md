...
- `memory_index_clear` / `memory_index_policy_update` / `memory_index_build` / `memory_index_rebuild` / `memory_index_merge` / `daily_log_index_rebuild` / `daily_log_index_run` — B 表 `memory_index_*`。LLM が要る構築は未索引データがあると fail loud。
