...
use opencrab_actions::webhook_target::{redact_webhook_url, validate_discord_webhook_url};
...
/// 自分の Nostr 転記設定を更新する。
///
/// URL は段階 A の `validate_discord_webhook_url`（Discord webhook のホスト許可リスト）で検証し、
...
pub(crate) fn set_my_nostr_relay(
    ...
    let url_arg = match args.get("webhook_url") {
        ...
        Some(serde_json::Value::String(s)) => {
            ...
            if trimmed.is_empty() {
                UrlArg::Clear
            } else if let Err(reason) = validate_discord_webhook_url(trimmed) {
