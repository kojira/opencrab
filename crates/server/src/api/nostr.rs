...
const NOSTR_GATEWAY_KIND: &str = "nostr";
...
pub async fn get_nostr_config(
    ...
    let running = state.gateways.is_running(NOSTR_GATEWAY_KIND, &id);
...
pub(crate) async fn apply_nostr_settings(
    ...
    if enabled {
        let gw = state.gateways.get(NOSTR_GATEWAY_KIND).ok_or_else(|| {
            (
    ...
    } else if let Some(gw) = state.gateways.get(NOSTR_GATEWAY_KIND) {
...
pub async fn generate_nostr_key(
    ...
    if let Some(gw) = state.gateways.get(NOSTR_GATEWAY_KIND) {
        gw.stop(&id).await;
    }
