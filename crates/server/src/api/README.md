...
| `GET /api/sessions/{id}` | logical または physical。open web binding があれば physical 状態 + `gateway_bound` + `web_binding_state` + `binding_address`（§3 写像の露出。DB 列追加なし）。各行に `agent_ids`。不在は 404 |
