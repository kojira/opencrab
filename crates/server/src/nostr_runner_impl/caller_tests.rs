...
#[test]
fn d_rb_001_two_enabled_agents_load_owner_from_schema_52_core_without_writes() {
    let conn = opencrab_db::init_memory().unwrap();
    assert_eq!(
    ...
    for (agent_id, owner) in [("agent-a", OWNER_HEX), ("agent-b", FRIEND_HEX)] {
        opencrab_db::queries::upsert_agent_nostr_config(
            &conn,
            &AgentNostrConfigRow {
                agent_id: agent_id.to_string(),
                secret_key: "enc:redacted".to_string(),
                relays_json: "[]".to_string(),
                filter_json: "{}".to_string(),
                enabled: true,
            },
        ...
        .unwrap();
        assert!(
            opencrab_db::queries::set_agent_nostr_owner_pubkey(&conn, agent_id, owner).unwrap()
        );
    }
    let before = conn.total_changes();
    assert_eq!(
        resolve_nostr_caller_identity(&conn, "agent-a", OWNER_HEX),
        CallerIdentity::Owner
    );
    assert_eq!(
        resolve_nostr_caller_identity(&conn, "agent-b", FRIEND_HEX),
        CallerIdentity::Owner
    );
    assert_eq!(
        conn.total_changes(),
        before,
        "authority lookup must be read-only"
    );
}
