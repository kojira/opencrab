...
pub fn build_agent_context(
    ...
    let prompt = format!(
        "You are {agent_name} ({persona}).\n\
         ...
         \n\
         A tool call is shown as `[tN>]`, followed by `[<tN] status:running` while its \
         background execution is still in progress. `running` is only an acknowledgement, \
         never the final result. Do not call that tool again merely because it is running; \
         end or wait for its terminal status.\n\
         \n\
         `[<tN] status:completed`, `failed`, `timed_out`, or `cancelled` is the authoritative \
         terminal result of the matching `[tN>]` call, and causes you to be called again. \
         Continue the task from that result. Calling the same tool again for the same request \
         starts a second, independent run. Do not repeat a completed call solely to satisfy \
         the original request again; call it again only when a distinct next operation or an \
         intentional retry is actually required.\n\
         \n\
