//! V3 §9 / DESIGN-WEBGATE §5.1: production Rust gateway を mock core に通す。
...
#[tokio::test]
async fn conformance_hello_bind_said_dedup() {
...
#[tokio::test]
async fn conformance_say_ok_external_rejected_close() {
    let dir = tempfile::tempdir().unwrap();
    let sock = dir.path().join("core.sock");
    let mut mock = MockCore::spawn(sock.clone());
    let connect = tokio::spawn(async move { connect_client(&sock).await });
...
#[tokio::test]
async fn conformance_activity_started_ended() {
    let dir = tempfile::tempdir().unwrap();
    let sock = dir.path().join("core.sock");
    let mut mock = MockCore::spawn(sock.clone());
    let connect = tokio::spawn(async move { connect_client(&sock).await });
...
#[tokio::test]
async fn conformance_frame_too_large_and_duplicate_close() {
    let dir = tempfile::tempdir().unwrap();
    let sock = dir.path().join("core.sock");
...
#[tokio::test]
async fn conformance_http_post_202_not_admitted_busy_and_old_routes_404() {
    let dir = tempfile::tempdir().unwrap();
    let sock = dir.path().join("core.sock");
    let mut mock = MockCore::spawn(sock.clone());
    let connect = tokio::spawn(async move { connect_client(&sock).await });
...
#[tokio::test]
async fn conformance_disconnect_and_unacked_503() {
    let dir = tempfile::tempdir().unwrap();
    let sock = dir.path().join("core.sock");
    let mut mock = MockCore::spawn(sock.clone());
    let connect = tokio::spawn(async move { connect_client(&sock).await });
...
#[test]
fn conformance_frame_grammar_unit() {
