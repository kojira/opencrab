...
| **Schedules (#455)** | `get_my_schedules`, `set_my_schedule` | all turns (`crates/server/src/agent_schedule.rs`) — an agent registers/reads its own cron / `@every` schedules for the current session (fires a message into the session on the central scheduler); not gated by `heartbeat_enabled` |
